...
If you are a commercial Sonatype IQ user, switch the data source to `iqServer`, and you can fill in the other extension settings.

However, the preferred method is to set your environment variables for authentication, and use a `.sonatyperc` file for the rest.
...
If you are able to login to IQ but don't have a token, you can create a [user token](https://help.sonatype.com/iqserver/automating/rest-apis/user-token-rest-api---v2) and set those values to `IQ_USERNAME` and `IQ_TOKEN`.
