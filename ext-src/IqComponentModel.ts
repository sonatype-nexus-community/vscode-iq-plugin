...
export class IqComponentModel {
  ...
  private determineWorkspaceDependencyType(
  ...
  private async packageAndConvertToNexusFormat(
  ...
  private async performIqScan() {
    try {
      ...
      const dependencyType = this.determineWorkspaceDependencyType(
        workspaceRoot
      );

      console.debug("Project dependency type:" + dependencyType);

      let data = await this.packageAndConvertToNexusFormat(
        dependencyType,
        workspaceRoot
      );
      ...
      if (undefined == data) {
        throw new RangeError(
          "Attempted to generated dependency list but received an empty collection. NexusIQ will not be invoked for this project."
        );
      }

      console.debug("getting applicationInternalId", this.applicationPublicId);
      let response: string = (await this.getApplicationId(
      ...
      for (let resultEntry of resultData.results) {
        ...
        if (dependencyType === DependencyType.NPM) {
          let coordinates = new NpmCoordinate(
            resultEntry.component.componentIdentifier.coordinates.packageId,
          ...
          let coordinates = new MavenCoordinate(
            resultEntry.component.componentIdentifier.coordinates.artifactId,
