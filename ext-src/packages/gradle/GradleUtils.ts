...
export class GradleUtils {
  ...
  private parseGradleDependencyTree(output: string): Array<GradlePackage> {
    console.debug(
      "------------------------------------------------------------------------------"
    );
    let dependencyList: GradlePackage[] = [];
    ...
    dependencyLines.forEach((dep) => {
      ...
      if (dep.trim().includes("\\---")) {
        ...
        if (artifact && group && version) {
          const dependencyObject: GradlePackage = new GradlePackage(
            artifact,
            group,
            version,
            "jar"
